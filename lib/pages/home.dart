...
class _HomeTabState extends State<HomeTab> {
  ...
  void loadTheme() async {
    ...
    if (!mounted) return;
    // Sync the global notifier with whatever was last saved, only if it
    // hasn't already been set elsewhere in this session.
    isDarkModeNotifier.value = prefs.getBool('darkMode') ?? isDarkModeNotifier.value;
  }

  void toggleTheme() async {
    final prefs = await SharedPreferences.getInstance();
    isDarkModeNotifier.value = !isDarkModeNotifier.value;
    await prefs.setBool('darkMode', isDarkModeNotifier.value);
  }
      ...
      items: [
        PopupMenuItem<String>(
          ...
          child: Row(
            children: [
              Icon(Icons.help,
                  color: isDarkMode ? Colors.white : const Color(0xFF1D2B4A), size: 22),
              const SizedBox(width: 12),
              Text('How to Use',
                  style: TextStyle(
                      color: isDarkMode ? Colors.white : const Color(0xFF1D2B4A),
                      fontSize: 16)),
            ],
          ),
        ),
      ...
      child: Scaffold(
        ...
        body: StreamBuilder<DatabaseEvent>(
          ...
          builder: (context, snapshot) {
            ...
            final double humidity = humidityRaw is num
                ? humidityRaw.toDouble()
                : double.tryParse(humidityRaw?.toString() ?? '') ?? 0;
                                  ...
                                  builder: (menuContext) {
                                    return IconButton(
                                      icon: const Icon(Icons.menu_rounded,
                                          color: Colors.white, size: 26),
                                      onPressed: () {
                                        _showMenu(menuContext, isDarkMode);
                                      },
                                    );
                                  },
                                ),
                    ...
                    child: Padding(
                      ...
                      child: Container(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            // ===== ROW 1: Temperature (left) + House (right, decorative) =====
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Expanded(
                            ...
                            // ===== ROW 2: Water Level (left) + Humidity + Flood Status (right) =====
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                // ----- Water Level column -----
                                ...
                                // ----- Humidity + Flood Risk Status column -----
                                Expanded(
                                  child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.center,
                                    children: [
                                      ...
                                      CustomPaint(
                                        ...
                                        painter: _HumidityGaugePainter(
                                          percent: (humidity / 100).clamp(0, 1).toDouble(),
                                          isDark: isDarkMode,
                                        ),
                                        child: SizedBox(
                                          width: 150,
...
class _WaterBucketPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final fillPaint = Paint()
      ..color = isDark ? Colors.blue.shade900.withOpacity(0.6) : Colors.blue.shade100;
    ...
    canvas.clipPath(Path()
      ...
      ..lineTo(right, top)
      ..close());
    canvas.drawRect(Rect.fromLTRB(left, fillTop, right, bottom), fillPaint);
    canvas.restore();
  ...
  bool shouldRepaint(covariant _WaterBucketPainter oldDelegate) =>
      oldDelegate.level != level || oldDelegate.isDark != isDark;
}
